...
@Component({
  selector: 'app-vendor-qc-list',
  templateUrl: './vendor-qc-list.component.html',
  styleUrls: ['./vendor-qc-list.component.css']
})
export class VendorQcListComponent implements OnInit {
              ...
              this.inBucketColumnDefinitions = [ 
                  ...
                  },
                  {
                      ...
                      onCellClick: (e: Event, args: OnEventArgs) => {
                          ...
                          sessionStorage.setItem('previousPath', '/app/vendorOnboardingQcList');

                          this.router.navigate(['app/onboarding/vendorOnboarding_qc'], {
                              queryParams: {
                                  "Idx": btoa(args.dataContext.Id),
                                  "Cdx": btoa(args.dataContext.CandidateId),
                              }
      ...
      this.onboardingApi.getOnboardingListingInfo(CandidateListingScreenType.All, this.RoleId, JSON.stringify(searchObj)).subscribe((data) => {
          ...
          if (apiResult.Result != "") {
              ...
              if(this.BusinessType != 3 &&  this.searchDataset.length > 0){
                  this.searchDataset = this.searchDataset.filter(a=>a.ClientId == Number(this.sessionService.getSessionStorage("default_SME_ClientId")))
              }
              // this.loadingScreenService.stopLoading();
              return;
          }
