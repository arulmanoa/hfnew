...
import { AlertService, ExcelService, OnboardingService, SessionStorage } from 'src/app/_services/service';
import { LoginResponses } from 'src/app/_services/model';


@Component({
  selector: 'app-salaybreakupdetails',
  templateUrl: './salaybreakupdetails.component.html',
  styleUrls: ['./salaybreakupdetails.component.css']
})
export class SalaybreakupdetailsComponent implements OnInit {
  ...
  @Input() IsMinimumWageCheckNotRequired : boolean = false;
  ...
  closeModal() {
    ...
    if (this.fromComponent == 'SalaryCalculator') {
    ...
    this.jsonObj.forEach(element => {
      ...
      if (this.lstOfPaygroupOverrideProducts != null && this.lstOfPaygroupOverrideProducts.length > 0) {
        ...
        if (isExistProduct != undefined) {
          ...
          console.log('PRODUCT GROUP ITEMS :: ', lstProductGroup);
  ...
  doCalculateCTC(): void {
    this._build_PaygroupOverride().then((result) => {
      ...
      this._newCandidateOfferDetails.LstCandidateRateSet[0].LstRateSet = this.jsonObj;
      this._newCandidateOfferDetails.IsMinimumWageCheckNotRequired = this.IsMinimumWageCheckNotRequired;
      console.log('REC ::', this._newCandidateOfferDetails);
      console.log('MIN REG ::',  this.IsMinimumWageCheckNotRequired);
  ...
  reviewLater() {
    ...
    this.alertService.confirmSwal("Confirmation?", "This will clear the current salary breakup, however you must calculate breakup before submitting this request.", "OK, Yes").then(result => {
      ...
      this._newCandidateOfferDetails.IsRateSetValid = false;
      this._newCandidateOfferDetails.IsMinimumwageAdhere = false;
