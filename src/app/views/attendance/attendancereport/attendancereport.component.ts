...
export class AttendancereportComponent implements OnInit {
  ...
  GetDirectReportingEmployee() {
    ...
    let searctElements: SearchElement[] = [
      {
        FieldName: "@userId",
        Value: this.RoleCode != "Manager" ? this.managerId : this.UserId
      },
      {
        FieldName: "@roleId",
        Value: this.RoleId
  ...
  GetPayPeriodsByManagerId() {
    ...
    console.log('payper', searctElements);
  ...
  doSearch() {
    ...
    this.LstEmployee = [];
    this.ClonedEmployeeList = [];
    if (_empId == '-200' || _empId == this.UserId) {
      this.PayPeriodtoBeHidden = false;
      spName = "GetAttendanceReportByUserAndDate";
      parameterObject = [{
        FieldName: "@userId",
        Value: this.RoleCode != "Manager" ? this.managerId : this.UserId
      },
  ...
  doExport() {
    ...
    this.LstEmployee.forEach(item => {
      ...
      exportExcelDate.push({
        ...
        // Type:  item.IsHoliday == 1 ? 'H' : (!item.IsWeekOff && item.IsHoliday == 0 &&
        //   item.FirstHalfEntitlementCode != 'WeekOff' && item.FirstHalfEntitlementCode != 'WO' &&
        //   item.FirstHalfEntitlementCode != '') ? item.FirstHalfEntitlementCode : (item.FirstHalfEntitlementCode == 'WeekOff' ||
        //      item.FirstHalfEntitlementCode == 'WO' || item.IsWeekOff) ? 'WO' : 
        //   (item.FirstHalfEntitlementCode != '' && !item.IsWeekOff && item.IsHoliday == 0 && item.FirstHalfEntitlementCode != 'WeekOff' && item.FirstHalfEntitlementCode != 'WO'
        //     && item.FirstHalfEntitlementCode != null && item.SecondHalfEntitlementCode != null && item.FirstHalfEntitlementCode == item.SecondHalfEntitlementCode) ?
        //     item.FirstHalfEntitlementCode : (item.FirstHalfEntitlementCode != '' && !!item.IsWeekOff && item.IsHoliday == 0 && item.FirstHalfEntitlementCode != 'WeekOff'
        //     && item.FirstHalfEntitlementCode != 'WO' && item.FirstHalfEntitlementCode != null && item.SecondHalfEntitlementCode != null && item.FirstHalfEntitlementCode != item.SecondHalfEntitlementCode) ?
        // item.FirstHalfEntitlementCode : (!item.IsWeekOff && item.IsHoliday == 0 && item.FirstHalfEntitlementCode == '' && item.SecondHalfEntitlementCode == '') ? item.FirstHalf : 'L',

    // Type: element.IsHoliday == 1 ? 'H' : (element.IsHoliday == 0 && element.IsWeekOff == 1 || element.FirstHalfEntitlementCode == 'WeekOff' ||
    //   element.FirstHalfEntitlementCode == 'WO') ? 'WO' : (element.IsHoliday == 0 && element.IsWeekOff == 0 && element.FirstHalfEntitlementCode != 'WeekOff' ||
    //     element.FirstHalfEntitlementCode != 'WO' && element.FirstHalfEntitlementCode != '') ? 'P' : 'L',
