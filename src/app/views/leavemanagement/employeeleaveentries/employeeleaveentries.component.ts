...
@Component({
  selector: 'app-employeeleaveentries',
  templateUrl: './employeeleaveentries.component.html',
  styleUrls: ['../employeeleaveentries/onlineattendnace.scss', './employeeleaveentries.component.scss']
})
export class EmployeeleaveentriesComponent implements OnInit, OnDestroy {
...
` : '---';
  ...
  async get_EmployeeEntitlementList() {
    try {
      ...
      if (apiResultProfile.Status && apiResultProfile.Result != null) {
        ...
        const entitlementListResult = await this.attendanceService.GetEmployeeEntitlementList(this._employeeId, EntitlementType.Leave)
          .toPromise();

        console.log('RES ENTITLEMENTLIST::', entitlementListResult);
  ...
  async GetEntitlementAvailmentRequests() {
    try {
      const result = await this.attendanceService.GetEntitlementAvailmentRequests(this._employeeId)
        .pipe(takeUntil(this.destroy$))
        .toPromise();
  ...
  getMinDateForAdditionalDateInput() {
    let currentDate = new Date();
    //currentDate.setDate(currentDate.getDate() + 1);
    currentDate.setDate(currentDate.getDate() - this.employeeEntitlement.Definition.MaxPastDaysForAdditionalDate)
    return currentDate;
  }
  ...
  onChange_Entitlement(event) {
    ...
    if (event === undefined) {
      ...
      // to show min date validation for additional date input 
      if (this.employeeEntitlement.Definition.MaxPastDaysForAdditionalDate !== undefined && this.employeeEntitlement.Definition.MaxPastDaysForAdditionalDate >= 0) {
        this.minDateForAdditionalDateInput = this.getMinDateForAdditionalDateInput();
      }
  ...
  do_editAppliedRequest(rowData) {
    ...
    // if (rowData.Status != EntitlementRequestStatus.Rejected && rowData.Status != EntitlementRequestStatus.Cancelled && rowData.Status != EntitlementRequestStatus.Approved) {
    // this.requestedDays = 0;
    // this.createForm();
    // this.CurrentDateTime = moment.utc().format('YYYY-MM-DD HH:mm:ss.SSS')
    // console.log('Crurent time : ', this.CurrentDateTime);
    ...
    this.leaveForm.patchValue({
      ...
      AdditionalDateInput: rowData.hasOwnProperty('AdditionalDate') ? new Date(rowData.AdditionalDate) : null,
  ...
  do_cancelAppliedRequest(item: EntitlementAvailmentRequest) {
    ...
    if (item.Status != EntitlementRequestStatus.Rejected && item.Status != EntitlementRequestStatus.Cancelled && item.Status != EntitlementRequestStatus.Approved) {
      $('#popup_edit_attendance').modal('hide');
      this.alertService.confirmSwal1("Confirmation", `You have applied for ${item.AppliedUnits} day(s) leave from ${moment(new Date(item.AppliedFrom)).format('DD-MM-YYYY')} to ${moment(new Date(item.AppliedTill)).format('DD-MM-YYYY')}. Are you sure you want to Cancel?`, "Yes, Cancel Now", "No, Abort").then((result) => {
        ...
        swalWithBootstrapButtons.fire({
          ...
          if (inputValue.value) {
            ...
            try {
              ...
              this.attendanceService.CancelEntitlementAvailmentRequest(entitlementAvailmentRequest)
                .subscribe((result) => {

                  let apiResult: apiResult = result;
                  console.log('SUBMITTED RES ::', result);
                  if (apiResult.Status) {
                    this.alertService.showSuccess('Your leave request has been cancelled successfully!');
      ...
      // this.alertService.showWarning('Action was blocked : Invalid Leave request - that the leave request is already approved/rejected.')
      this.alertService.showWarning(`This leave request has already been ${EntitlementRequestStatus[item.Status].toLowerCase()}`)
      return;
    }
  ...
  cancelApprovedRequest(selectedItems = []) {
    ...
    if (selectedItems.filter(a => a.Status != 400).length > 0) {
      // this.alertService.showWarning('Action was blocked : Invalid Leave request - that leave request is not in approved status');
      this.alertService.showWarning('This leave request is not in an approved status');
      return;
    }
  ...
  callNumberOfDaysForAppliedLeaveAPI() {
   ...
   if (this.leaveForm.get('compOffDates').value && this.employeeEntitlement && this.employeeEntitlement.Definition.IsCompOff) {
    ...
    console.log('ENTILMENT REQUEST :: ', entitlementAvailmentRequest);
    this.attendanceService.putNumberOfDaysForAppliedLeave(entitlementAvailmentRequest).subscribe((result) => {
      let apiResult: apiResult = result;
