...
@Component({
  ...
  styleUrls: ['../employeeleaveentries/onlineattendnace.scss', './teamleaverequest.component.scss']
})
export class TeamleaverequestComponent implements OnInit, OnDestroy {
  ...
  onRefresh() {
    ...
    this.code = 'entitlementAvailmentRequest';
  ...
  Get_EntitlementAvailmentRequestsForApproval() {
    this.attendanceService.GetEntitlementAvailmentRequestsForApproval()
      .pipe(takeUntil(this.destroy$))
      .subscribe(
        (result) => {
          console.log('RES ENTITLEMENTLIST REQ APPROVALS ::', result);
  ...
  bulk_approve_reject(whichaction) {
    ...
    if (whichaction == true && this.inEmployeesInitiateSelectedItems.filter(a => a.Status == 400).length > 0) {
      this.alertService.showWarning('Leave request cannot be processed as it has already been approved.')
      return;
    }

    if (whichaction == false && this.inEmployeesInitiateSelectedItems.filter(a => a.Status == 300).length > 0) {
      this.alertService.showWarning('Leave request cannot be processed as it has already been rejected.')
      return;
    }
    ...
    if (IsAllowToCancel == true && this.inEmployeesInitiateSelectedItems.filter(a => a.Status == 300 || a.Status == 400).length > 0) {
      ...
      this.alertService.showWarning(`Leave request cannot be canceled - one or more requests for leave have no authority to cancel the ${textmsg}  leave.`);
      return;
    }
