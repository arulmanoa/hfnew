...
import { ExpenseBatch, ExpenseClaimRequestStatus, SubmitExpenseBatchModel, MigrateExpenseModel, SubmitExpenseClaimRequestModel } from 'src/app/_services/model/Expense/ExpenseEligibilityCriteria';
...
@Component({
  selector: 'app-reimbursementforfinance',
  templateUrl: './reimbursementforfinance.component.html',
  styleUrls: ['./reimbursementforfinance.component.css']
})
export class ReimbursementforfinanceComponent implements OnInit {
              ...
              try {
                ...
                if (resultObj.Status && resultObj.Result != null) {
                  ...
                  if (this.expenseConfiguration.ProductConfigurationList != null && this.expenseConfiguration.ProductConfigurationList.length > 0) {
                    ...
                    let expenseBatch = args.dataContext;
                    ...
                    expenseBatch.ExpenseClaimRequestList = expenseBatch.ExpenseClaimRequestList != null && expenseBatch.ExpenseClaimRequestList.length > 0 ?
                      expenseBatch.ExpenseClaimRequestList.filter(a => a.Status == 300 || a.Status == 400) : [];
  ...
  getExpenseData() {
    ...
    this.searchedClientId = this.sessionService.getSessionStorage("default_SME_ClientId");
    
    if (this.searchConfigurationForexpense.SearchElementList.find(item => item.FieldName == '@clientContractId') != undefined) {
      this.searchConfigurationForexpense.SearchElementList.find(item => item.FieldName == '@clientContractId').Value = this.sessionService.getSessionStorage("default_SME_ContractId");
    }
  ...
  onClickCreatePayoutBatch() {
    ...
    this.alertService.confirmSwal1("Confirmation", `Are you sure you want to Process it`, "Yes, Confirm", "No, Cancel").then((result) => {
      ...
      if (expenseClaimRequests.length > 0 && expenseClaimRequests.filter(a => a.Status == 400).length == 0) {
        this.alertService.showWarning("There is no approved claim request for this employee.");
        return;
      }

      expenseClaimRequests = expenseClaimRequests.length > 0 ? expenseClaimRequests.filter(a => a.Status == 400) : [];
  ...
  triggerSubmitExpense(ExpenseClaimRequestList, RequestedExpenseClaimList) {

    this.loadingSreenService.startLoading();
    let approvedExpenseItems = [];
    approvedExpenseItems = ExpenseClaimRequestList;

    var sum = 0;
    var sum1 = 0;
    let rejectedItemIds = [];

    RequestedExpenseClaimList.forEach(e => {
      sum += parseInt(e.RequestedAmount);
      sum1 += parseInt(e.ApprovedAmount)
      e.Status == 300 ? rejectedItemIds.push(e.Id) : true;
    });

    approvedExpenseItems.forEach(e => { e.ModeType = UIMode.Edit });

    var expenseBatch = new ExpenseBatch();
    expenseBatch.EmployeeId = this.rowData.EmployeeId;
    expenseBatch.Name = this.rowData.Name;
    expenseBatch.TotalRequestedAmount = sum;
    expenseBatch.TotalApprovedAmount = sum1;
    expenseBatch.TotalNoOfDocuments = RequestedExpenseClaimList.length;
    expenseBatch.TotalNoOfRequests = RequestedExpenseClaimList.length;
    expenseBatch.RejectedClaimIds = [];
    expenseBatch.Status = 1;
    expenseBatch.ExpenseClaimRequestList = approvedExpenseItems;
    expenseBatch.ModeType = UIMode.Edit;
    expenseBatch.Id = this.rowData.Id;
    ...
    submitExpenseBatchModel.ModuleProcessAction = 0;
  ...
  submitExpenseRequest(action) {
    ...
    if (this.rowData.ExpenseClaimRequestList.filter(a => a.Status == 300).length == 0) {
      this.triggerSubmitExpense(this.rowData.ExpenseClaimRequestList, this.rowData.ExpenseClaimRequestList);
      return;
    }



    let selectedClaimReqs = [];
    selectedClaimReqs = this.rowData.ExpenseClaimRequestList.filter(a => a.Status == 300); // only rejected status



    if (selectedClaimReqs.length == 0) {
    ...
    if (selectedClaimReqs.length > 0) {
      if ((selectedClaimReqs.filter(a => a.Status <= 200).length > 0)) {
      ...
      var sum = 0;
      var sum1 = 0;
      selectedClaimReqs.forEach(e => {
        sum += parseInt(e.RequestedAmount);
        sum1 += parseInt(e.ApprovedAmount)

      });

      this.alertService.confirmSwal1("Are you sure you want to submit ", `No of Rejected Claims : ${selectedClaimReqs.length}`, "Yes, Confirm", "No, Cancel").then((result) => {
        this.loadingSreenService.startLoading();
        selectedClaimReqs.forEach(e => { e.ModeType = UIMode.Edit });
        selectedClaimReqs.forEach(e1 => {
          if (e1.RejectionRemarks) {
            let Remarks = `Employee : ${e1.Remarks} | Finance : ${e1.RejectionRemarks}`
            e1.Remarks = Remarks;
          }
        });
        ...
        var submitExpenseClaimRequestModel = new SubmitExpenseClaimRequestModel();
        submitExpenseClaimRequestModel.ExpenseClaimRequestList = selectedClaimReqs;
        ...
        this.reimbursementService.SubmitExpenseClaimRequest(submitExpenseClaimRequestModel)
          .subscribe((expenseClaimReqgObj) => {
            try {
              ...
              if (resultObj.Status && resultObj.Result != null) {
                this.loadingSreenService.stopLoading();
                if (this.rowData.ExpenseClaimRequestList.filter(a => a.Status == 400).length > 0) {
                  this.triggerSubmitExpense(this.rowData.ExpenseClaimRequestList.filter(a => a.Status == 400), this.rowData.ExpenseClaimRequestList);
                  return;
                }
                this.alertService.showSuccess(resultObj.Message);
                this.expenseClaimRequestSlider = false;
